...
#   pm15m      Polymarket 15-minute market -> pm_window_15m/pm_quote_15m, tailed from pmqb's
#              standalone 15m capture (backend.data.ingest_pm15m). Outcomes arrive in that
#              file as the capture resolves each window; after a capture outage run
#              `python3 -m backend.data.ingest_pm15m --backfill-gamma` once to fill the gap.
...
#   pmdata15m  The same for the 15-minute markets (--series btc-15m) -> pm_l2_*_15m.
#              Shares the pmdata lock, so the two never write to market.db at once.
#              Each new day costs 48 PMData quota units (a 5m day costs 144), and a
#              missed week costs up to 7x that on the next run. Deliberately NOT in
#              `all`, so a manual `all` never spends 15m quota by surprise.
...
PM15M_LOCK=/tmp/btc10_ingest_pm15m.lock
...
case "${1:-all}" in
  ...
  pm15m)
    exec /usr/bin/flock -n "$PM15M_LOCK" python3 -m backend.data.ingest_pm15m ;;
  binance1s)
  ...
  pmdata15m)
    exec /usr/bin/flock -n "$PMDATA_LOCK" \
      python3 -m backend.data.ingest_pmdata --series btc-15m "${pmdata_args[@]}" ;;
  all)
    ...
    /usr/bin/flock -n "$PM15M_LOCK"   python3 -m backend.data.ingest_pm15m || true
  ...
  *)
    echo "usage: $0 {stream|twap|pm15m|binance1s|binance|binance1m|pmdata|pmdata15m|all}" >&2; exit 2 ;;
esac
